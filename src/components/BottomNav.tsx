...
export default function BottomNav() {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-[950] pointer-events-auto border-t border-border/40 bg-card/85 pb-safe backdrop-blur-lg shadow-[0_-8px_30px_rgba(0,0,0,0.08)] rounded-t-[1.75rem] md:hidden"
    >
        ...
        {mainItems.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <li key={to} className="flex-1">
              ...
              >
                <div
                  className={`flex items-center justify-center rounded-full px-4 py-1 transition-all duration-300 ${active
                ...
                <span
                  className={`text-[9px] tracking-wide transition-colors ${active ? "text-primary font-black" : "text-muted-foreground font-semibold"
                    }`}
                >
                  {label}
                </span>
        ...
        <li className="flex-1">
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              ...
              >
                <div
                  className={`flex items-center justify-center rounded-full px-4 py-1 transition-all duration-300 ${isOpen
                      ? "bg-primary/10 text-primary scale-105 font-bold"
                      : "text-muted-foreground hover:text-foreground"
                    }`}
                >
                ...
                <span
                  className={`text-[9px] tracking-wide transition-colors ${isOpen ? "text-primary font-black" : "text-muted-foreground font-semibold"
                    }`}
                >
                  {t("more")}
                </span>
            ...
            <SheetContent
              side="bottom"
              className="rounded-t-[2.25rem] border-t border-border/40 bg-card p-6 pb-28 max-h-[85vh] overflow-y-auto shadow-[0_-15px_30px_rgba(0,0,0,0.12)] focus-visible:outline-none"
            >
              {/* Grab Handle */}
                ...
                {moreItems.map(({ to, icon: Icon, label, desc, color }) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center justify-between rounded-2xl border p-4 transition-all active:scale-[0.98] ${active
